...
class convolution2d(module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size: tuple, stride=1, padding=0, bias=True, device = None):
        ...
        self.filters = torch.randn(out_channels, in_channels, *kernel_size).to(device)
        self.bias = torch.randn(out_channels).to(device) if bias else None
    ...
    def backward(self, grad_output):
        '''
        grad_output has shape (n, out_channel, output_height, output_width)

        grad_input should have shape (n, in_channel, input_height, input_width)
        grad_weight should have shape (out_channel, in_channel, kernel_height, kernel_width)
        grad_bias should have shape (out_channel)

        the backward is very similar to the backward pass of a linear layer, so the only difference is that now we need to do a similar slicing up of X as before
        '''

        n, out_channels, output_height, output_width = grad_output.shape
        _, in_channels, _, _ = self.X.shape
        kernel_height, kernel_width = self.kernel_size

       
        X_unfolded = F.unfold(self.X, kernel_size=self.kernel_size, stride = self.stride) # X_unfolded has shape (n, in_channels * kernel_height * kernel_width, output_height * output_width)
        X_unfolded = X_unfolded.view(n, in_channels, kernel_height, kernel_width, -1) # X_unfolded has shape (n, in_channels, kernel_height, kernel_width, output_height * output_width)

        # dL/dX = dL/do * do/dX, where is the output
        # dL/do = grad_output is given. We need to find do/dX
